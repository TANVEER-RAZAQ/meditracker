...
public class VisitController {
    ...
    public ResponseEntity<Visit> recordVitals(@PathVariable Long visitId, @Valid @RequestBody VitalsRequest request) {
        Visit visit = visitService.recordVitals(visitId, request.getTemperatureCelsius(), request.getBpSystolic(), request.getBpDiastolic(), request.getHeartRate());
        return ResponseEntity.ok(visit);
    }
    ...
    public ResponseEntity<Visit> consultation(@PathVariable Long visitId, @Valid @RequestBody ConsultationRequest request) {
        Visit visit = visitService.addConsultation(visitId, request.getDiagnosis(), request.getMedications(), request.isTestsNeeded());
        return ResponseEntity.ok(visit);
    }
}
